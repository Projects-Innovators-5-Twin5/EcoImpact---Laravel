...
<div class="bg-white p-4">
    <div class="p-4 row">
        <div class="card m-4 shadow-lg col-md-7" style="border:none !important;padding: 0;">
            <img class="card-img-top" src="{{ asset('storage/' . $articles[0]['image']) }}" alt="{{ $articles[0]['title'] }}" style="max-height: 300px;">
            
            <div class="card-body">
                <p class="card-text">Energie Renouvelables</p>
                <a class="card-text" style="font-weight: bold; font-size:20px;">{{ Str::limit($articles[0]['content'], 100) }}</a>
            </div>
        </div>
    ...
    <div class="row p-4">
        ...
        <a href="{{ route('front.articles.show', $article['id']) }}" class="card m-2 col-12 col-md-perso smal-card" style="border:none !important;cursor: pointer; text-decoration: none;">
            <img class="card-img-top mt-4 rounded" style="max-height: 200px;" src="{{ asset('storage/' . $article['image']) }}" alt="{{ $article['title'] }}" class="img-fluid rounded mb-3">
            <div class="card-body" style="padding-left: 0 !important;">
                <p class="card-text">{{ $article['title'] }}</p>
                <p class="card-text" style="font-weight: bold">{{ Str::limit($article['content'], 100) }}</p>
            </div>
        </a>
        @endforeach
    </div>
